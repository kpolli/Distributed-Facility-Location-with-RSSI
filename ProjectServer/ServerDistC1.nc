...
implementation
{
  ...
  uint16_t temp4 = 10000;
  uint16_t temp8;
  uint16_t getRssi(message_t *msg);

  npacket_t clientC[2], clientU[2], clientB[2];
...
/*
if (omsg->id == 1){
  clientC[0].id = omsg->id;
  clientC[0].dist = sum1;
  clientC[0].masterID = TOS_NODE_ID;
  clientC[0].receiveRequest = FALSE;
  clientC[0].facility = FALSE;


}
else if (omsg->id == 2){ 
  clientC[1].id = omsg->id;
  clientC[1].dist = sum3;
  clientC[1].masterID = TOS_NODE_ID;
  clientC[1].receiveRequest = FALSE;
  clientC[1].facility = FALSE;
}
